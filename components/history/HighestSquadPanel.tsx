    ...
    <div className="md:col-span-2 bg-[#0d0e12] border border-zinc-900 rounded-2xl p-5 flex flex-col justify-between">
      <div>
        <span className="text-[9px] uppercase font-mono tracking-widest text-[#e8ff3b] block mb-1">
          {isMounted ? t('history_highest_squad') : '👑 COVETED HIGHEST-RATED SQUAD'}
        </span>
        <h2 className="font-display font-black text-sm text-zinc-200 uppercase tracking-tight">
          {record ? (
            ...
            (isMounted ? t('history_no_highest') : 'No Peak Squad Recorded Yet')
          )}
        </h2>
        ...
        {record && record.squadPlayers && record.squadPlayers.length > 0 ? (
          ...
          <p className="text-xs text-zinc-500 leading-relaxed mt-4">
            {isMounted ? t('history_no_highest_desc') : 'Complete drafting sequences and win the tournament simulation campaigns to register your dream rosters into the peak squad showcase block!'}
          </p>
        )}
      </div>
      ...
      {record && (
        <div className="flex gap-4 mt-4 pt-3 border-t border-zinc-900 font-mono text-[9px] text-zinc-500">
          <span>{isMounted ? t('history_tactic') : 'TACTIC'}: <strong className="text-zinc-300 font-bold uppercase">{record.tacticalStyle || 'Tiki-Taka'}</strong></span>
          <span>{isMounted ? t('history_formation') : 'FORMATION'}: <strong className="text-zinc-300 font-bold uppercase">{record.formation}</strong></span>
          <span>{isMounted ? t('history_clean_sheets') : 'CLEAN SHEETS'}: <strong className="text-emerald-400 font-bold uppercase">{record.cleanSheets ?? 'N/A'}</strong></span>
        </div>
      )}
    </div>
  );

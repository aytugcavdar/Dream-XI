    ...
    <div className="bg-zinc-950 border-2 border-emerald-500/30 p-5 rounded-2xl h-full flex flex-col justify-between shadow-[0_0_20px_rgba(16,185,129,0.15)] relative overflow-hidden">
      ...
      <div className="space-y-4 font-sans">
        <div className="flex items-center gap-2 border-b border-zinc-900 pb-3">
          ...
          <div>
            <h3 className="font-display font-black text-sm uppercase text-white tracking-wider leading-none">
              {isMounted ? t('build_complete_title') : 'All-Star Eleven Ready'}
            </h3>
            <span className="text-[10px] font-mono text-zinc-400 mt-1 block">
              {isMounted ? t('build_complete_desc') : 'Your squad is primed for World Cup simulation'}
            </span>
          </div>
        </div>
        ...
        <div className="p-2.5 text-[11px] font-mono rounded-lg bg-emerald-950/20 border border-emerald-900/40 text-emerald-300 leading-relaxed">
          {chemistryBreakdown.chemistry >= 80 ? (
            <span>{isMounted ? t('build_feedback_high') : <span>🔥 <strong>Outstanding Chemistry!</strong> Your icons share profound country & era bonds. This collection is destined to lift the World Cup!</span>}</span>
          ) : chemistryBreakdown.chemistry >= 50 ? (
            <span>{isMounted ? t('build_feedback_med') : <span>⚡ <strong>Solid Cohesion!</strong> Strong regional lines of communication will push this tactical structure deep into the knockout brackets.</span>}</span>
          ) : (
            <span>{isMounted ? t('build_feedback_low') : <span>⚙️ <strong>Decent Setup!</strong> Diverse generational talents are ready to compete, but may struggle against high-chemistry giants.</span>}</span>
          )}
        </div>
        ...
        {chemistryBreakdown.activePartnerships.length > 0 && (
          <div className="bg-zinc-900/40 border border-zinc-900 rounded-xl p-3 space-y-1.5">
            <div className="flex items-center gap-1.5 text-[10px] font-mono font-bold text-[#e8ff3b] uppercase tracking-widest">
              ...
              <span>{isMounted ? t('build_legendary_partnerships').replace('{count}', String(chemistryBreakdown.activePartnerships.length)) : `Legendary Partnerships (${chemistryBreakdown.activePartnerships.length})`}</span>
            </div>
        ...
        <div>
          <h4 className="font-mono text-[9px] text-zinc-500 uppercase tracking-widest mb-1.5">{isMounted ? t('build_selected_lineup') : 'Selected Lineup'}</h4>
      ...
      <div className="pt-4 border-t border-zinc-900 space-y-2 text-center">
        ...
        >
          ...
          <span>{isMounted ? t('build_launch_sim') : 'LAUNCH CUP SIMULATOR'}</span>
        </button>
        ...
        >
          ...
          <span>{isMounted ? t('build_disband_team') : 'Disband Team & Restart Draft'}</span>
        </button>
      </div>
    </div>
  );
